...
class LectureController extends Controller
{
    ...
    public function getLectures(Request $request, $categoryId=0)
    {
        ...
        if($categoryId == 0){
            // search query
            if($search && $search != ''){
                $data['lecture_list'] = AudioLecture::with('category', 'attachments')->where('status',1)->where('title', 'like', '%'.$search.'%')->get();
            }else{
                $data['lecture_list'] = AudioLecture::with('category', 'attachments')->where('status',1)->get();
            }
        }else{
            // search query
            if($search && $search != ''){
                $data['lecture_list'] = AudioLecture::with('category', 'attachments')->where('status',1)->where('category_id',$categoryId)->where('title', 'like', '%'.$search.'%')->get();
            }else{
                $data['lecture_list'] = AudioLecture::with('category', 'attachments')->where('status',1)->where('category_id',$categoryId)->get();
            }
        }
