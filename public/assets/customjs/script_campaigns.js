function getCampaignsPageData(){
    ...
    SendAjaxRequestToServer(type, url, data, '', getCampaignsPageDataResponse, '', '');
}
    ...
    if (campaignList.length > 0) {
        $.each(campaignList, function (index, campaign) {
            ...
            html += `<tr>
                        ...
                        <td class="text-start text-nowrap">${campaign.title}</td>
                        <td class="text-start text-nowrap">${campaign.target_amount}</td>
                        <td class="text-start text-nowrap">${trimText(campaign.description, 50)}</td>
                        <td class="text-start text-nowrap">${formatDate(campaign.date)}</td>
                        ...
                        <td class="text-start text-nowrap">
                            <div class="btn-group">
                                ...
                                <ul class="dropdown-menu dropdown-menu-custom">
                                    <a class="dropdown-item" href="javascript:;" onclick="editCampaign(${campaign.id})">
                                    ...
                                    <a class="dropdown-item" href="javascript:;" onclick="deleteCampaignConfirm(${campaign.id})">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24">
                                            <g fill="none">
...
function saveCampaign(){
    ...
    SendAjaxRequestToServer(type, url, data, '', saveCampaignResponse, '', '#saveCampaign_btn');
}
    ...
    if (response.status == 200  || response.status == '200') {
        ...
        if(campaignDetail != null){
            ...
            if(campaignDetail.thumbnail != null){
                $(".thumbnail_preview").attr('src', campaignDetail.thumbnail).show();
            }else{
                $(".thumbnail_preview").attr('src', '').hide();
            }
            ...
            if (tasks.length > 0) {
                $.each(tasks, function (index, task) {
                    ...
                    html += `<div class="d-flex align-items-center justify-content-between task_div" id="task_div_${task.id}">
                                <div class="form-floating col-5 my-2">
                                    <input type="hidden" name="tasks[${index+1}][id]" value="${task.id}">
                                    <input class="form-control" type="text" id="task_title${index+1}" name="tasks[${index+1}][title]" value="${task.title}" placeholder="Enter Title ${index+1}">
                                    <label class="ms-2" for="task_title${index+1}">Title ${index+1}</label>
                                </div>
                                <div class="form-floating col-5 my-2">
                                    <input class="form-control" type="text" id="task_amount${index+1}" name="tasks[${index+1}][amount]" value="${task.task_amount}" placeholder="Enter Amount ${index+1}">
                                    <label class="ms-2" for="task_amount${index+1}">Amount ${index+1}</label>
                                </div>
                                <svg class="cross-svg" onclick="deleteCampaignTaskConfirm(${task.id})" xmlns="http://www.w3.org/2000/svg" width="0.9em" height="0.9em" viewBox="0 0 15 15">
...
function deleteCampaignTaskConfirmed(){
    ...
    // PASSING DATA TO FUNCTION
    SendAjaxRequestToServer(type, url, data, '', deleteCampaignTaskConfirmedResponse, '', '');
}
...
function deleteCampaignConfirmed(){
    ...
    // PASSING DATA TO FUNCTION
    SendAjaxRequestToServer(type, url, data, '', deleteCampaignConfirmedResponse, '', '');
}
