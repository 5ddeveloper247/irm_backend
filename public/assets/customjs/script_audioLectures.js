...
function getAudioLecturesPageData(formValues = {}) {
    ...
    SendAjaxRequestToServer(
        ...
        data,
        "",
        getAudioLecturesPageDataResponse,
        "",
        ""
    );
}
...
function makeAudioCategoryListing(categoryList) {
    var html = "";
    // empty
    $("#audioCategory_table_body").html("");
    // audioCategory_table table destroy and empty
    if ($.fn.DataTable.isDataTable("#audioCategory_table")) {
        $("#audioCategory_table").DataTable().destroy().clear();
    }
    if (categoryList.length > 0) {
        $.each(categoryList, function (index, category) {
            html += `<tr>
                        ...
                        <td class="text-start text-nowrap">
                            <div class="btn-group">
                                ...
                                <ul class="dropdown-menu dropdown-menu-custom">
                                    <a class="dropdown-item" href="javascript:;" onclick="editAudioCategory(${
                                        category.id
                                    })">
...
function makeAudioLectureListing(lectureList) {
    ...
    var html = "";
    // audioLecture_table table destroy and empty
    if ($.fn.DataTable.isDataTable("#audioLecture_table")) {
        $("#audioLecture_table").DataTable().destroy();
        $("#audioLecture_table_body").html("");
    }
            ...
            html += `<tr>
                        ...
                        <td class="text-start text-nowrap">
                            <div class="btn-group">
                                ...
                                <ul class="dropdown-menu dropdown-menu-custom">
                                    <a class="dropdown-item" href="javascript:;" onclick="editAudioLecture(${
                                        lecture.id
                                    })">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 16 16">
                                            <path fill="currentColor" d="M15.49 7.3h-1.16v6.35H1.67V3.28H8V2H1.67A1.21 1.21 0 0 0 .5 3.28v10.37a1.21 1.21 0 0 0 1.17 1.25h12.66a1.21 1.21 0 0 0 1.17-1.25z" />
                                    ...
                                    <a class="dropdown-item" href="javascript:;" onclick="deleteAudioLectureConfirm(${
                                        lecture.id
                                    })">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24">
                                            <g fill="none">
...
function deleteAudioCategoryConfirmed() {
    ...
    SendAjaxRequestToServer(
        ...
        data,
        "",
        deleteAudioCategoryConfirmedResponse,
        "",
        ""
    );
}
    ...
    if (response.status == 200 || response.status == "200") {
        ...
        if (lectureDetail != null) {
            ...
            if (lectureDetail.thumbnail != null) {
                $(".thumbnail_preview")
                    .attr("src", lectureDetail.thumbnail)
                    .show();
            } else {
                $(".thumbnail_preview").attr("src", "").hide();
            }
...
function deleteAudioLectureAttConfirmed() {
    ...
    SendAjaxRequestToServer(
        ...
        data,
        "",
        deleteAudioLectureAttConfirmedResponse,
        "",
        ""
    );
}
...
function deleteAudioLectureConfirmed() {
    ...
    SendAjaxRequestToServer(
        ...
        data,
        "",
        deleteAudioLectureConfirmedResponse,
        "",
        ""
    );
}
    ...
    if (selectedFiles.length < 8) {
        ...
        selectedFiles.forEach((file, index) => {
            ...
            reader.onload = function (e) {
                ...
                const $image = $("<img>")
                    .attr("src", "/assets/images/audio-placeholder.png")
                    .addClass("img-prev");
...
$(document).ready(function () {
    $("#search_filter").on("keyup", function () {
        ...
        $("#audioCategory_table_body tr").each(function () {
            var row = $(this);
            var hasMatch = false;
    ...
    $("#search_filter_02").on("keyup", function () {
        ...
        // Iterate through each row
        $("#audioLecture_table_body tr").each(function () {
            var row = $(this);
            var hasMatch = false;
        ...
        // Display 'No result found' if no rows are visible
        if ($("#audioLecture_table_body tr:visible").length === 0) {
            var no_result_row = `
                <tr class="no_result_row">
                    <td class="text-start text-danger text-center" colspan="8">No result found</td>
                </tr>`;
            $("#audioLecture_table_body").append(no_result_row);
        }
        // remove table-highlight class from all td when input is empty
        if ($(this).val() == "") {
            $(".no_result_row").remove();
            // remove table-highlight class from all td
            $("#audioLecture_table_body tr td").removeClass("table-highlight");
        }
    });
});
